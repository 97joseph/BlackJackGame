...
public class Player implements Serializable {
	 ...
	 * }
	 * 
	 * // Adds a player's bet from their bank if they win. Sets bet to zero
	 * afterwards.
	 * public void win(int betMultiplier) {
	 * bank += (bet*betMultiplier);
	 * if(betMultiplier>1){
	 ...
	 * }
	 * 
	 * // Sets a player's bet to 0 if the "push". Notice, no bet is added or
	 * removed.
	 * public void push() {
	 * pushCounter++;
	 * }
	...
	public char getMove(int dealerCard, int index) {
		...
		if (!getHand(index).canSplit() && !getHand(index).isSoft()) {
			...
			if (getTotal(index) <= 8) {
				options = "HHHHHHHHHH";
			} else if (getTotal(index) == 9) {
				options = "HEEEEHHHHH";
			} else if (getTotal(index) == 10) {
				options = "EEEEEEEEHH";
			} else if (getTotal(index) == 11) {
				options = "EEEEEEEEEH";
			} else if (getTotal(index) == 12) {
				options = "HHTTTHHHHH";
			} else if (getTotal(index) == 13) {
				options = "TTOOOHHHHH";
			} else if (getTotal(index) == 14) {
				options = "OOOOOHHHHH";
			} else if (getTotal(index) == 15) {
				options = "OOOOOHHHHH";
			} else if (getTotal(index) == 16) {
				options = "OOSSSHHHHH";
			} else if (getTotal(index) == 17) {
				options = "SSSSSSSOOO";
			} else {
				options = "SSSSSSSSSS";
			}
		...
		// Soft
			...
			if (getTotal(index) == 13) {
				...
				options = "HEEEEHHHHH";
			} else if (getTotal(index) == 18) {
				options = "TFFFFTTTTT";
			} else if (getTotal(index) == 19) {
				options = "OOOOOOOOTO";
			} else if (getTotal(index) == 20) {
				options = "OOOOOOOOOO";
			} else if (getTotal(index) == 21) {
				options = "OOOOOOOOOO";
			}
		}
		// Splits
			...
			if (getTotal(index) == 4) {
				options = "QQPPPPHHHH";
			} else if (getTotal(index) == 6) {
				options = "QQPPPPHHHH";
			} else if (getTotal(index) == 8) {
				options = "HHHQQHHHHH";
			} else if (getTotal(index) == 12) {
				options = "QPPPPHHHHH";
			} else if (getTotal(index) == 14) {
		...
		// System.out.println("Move chosen for P2L: " +
		// options.charAt(Math.floorMod(dealerCard-2,10)));
		...
		if (move == 'O') {
			move = (getHand(index).getNoOfCards() == 6) ? 'H' : 'S';
		}
		if (move == 'T') {
			move = (getHand(index).getNoOfCards() >= 5) ? 'H' : 'S';
		}
		if (move == 'Q') {
			move = 'P';
		}
